...
const AsalKapalTable = ({ data = [], onEdit, onDelete }) => {
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
        ...
        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
          {data.length > 0 ? (
            data.map((item, index) => (
              <tr key={item.id_asal_kapal}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{index + 1}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{item.nama_asal_kapal}</td>
            ...
            <tr>
              <td colSpan="3" className="px-6 py-8 text-center text-gray-500 italic">Tidak ada data kedudukan kapal.</td>
            </tr>
          )}
        </tbody>
      </table>
